 ...
 * 
 */
public class AstroActivityHealthChecker implements
		HealthChecker<AstroActivity> {

	public boolean canVisit(Object o) {
		// Return True if we can visit the object. We could do
		// deeper (but not time consuming) checks here, for instance
		// if the health checker only deals with AstroActivity where
		// a certain configuration option is enabled.
		return o instanceof AstroActivity;
	}
	...
	public VisitReport visit(AstroActivity activity, List<Object> ancestry) {
		AstroActivityConfigurationBean config = activity.getConfiguration();
		...
		return new VisitReport(HealthCheck.getInstance(), activity,
				"Astro service OK", HealthCheck.NO_PROBLEM, subReports);
	}
